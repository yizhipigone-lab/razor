...
# -*- coding: utf-8 -*-
"""
MA5 角度策略 �?5分钟线精确回�?(2023-01-01 ~ 2026-05-02)
...
def load_intraday_data(code: str, d: date) -> pd.DataFrame:
    """
    加载单只股票某一天的日内数据
    优先�? QMT 1分钟 > TDX+QMT合并5分钟 > baostock 5分钟缓存
    """
    # 尝试 QMT 1分钟数据
    ...
    if qmt_path.exists():
        try:
            ...
            if 'datetime' in df.columns and not df.empty:
                ...
                if day_mask.sum() >= 48:  # 至少有半天数�?                    result = df[day_mask].copy()
...
def check_intraday_stop(pos: Position, bar: dict, d: date) -> Optional[Tuple]:
    ...
    # 1. 硬止�?-5.5%: 用这根K线的Low检�?    if low <= pos.entry_price * (1 + HARD_STOP):
        exit_px = pos.entry_price * (1 + HARD_STOP)
        return (exit_px, f"硬止�?{(exit_px/pos.entry_price-1)*100:.1f}%)", None)
...
def run_intraday_backtest():
    ...
    for day_idx, d in enumerate(trading_dates):
        ...
        for code, pos in list(engine.positions.items()):
            ...
            if intraday.empty:
                # baostock 实时下载已禁用（使用预缓存的 merged min5 数据�?                missing_data_count += 1
                # 没有日内数据：用日线Low检查硬止损，Close检查其�?                dc = daily_closes.get(d, {}).get(code)
                if dc is not None and dc > 0:
                    dh = daily_highs.get(d, {}).get(code, dc)
                    # 用日最低价模拟
                    day_bar = daily_closes.copy()
                    # 获取该股票在当日的日�?                    stock_row = bt_bars[(bt_bars['code']==code) & (bt_bars['date']==d)]
                    ...
                    # EOD检�?                    eod_result = check_eod_stop(pos, dc, d)
                    if eod_result:
        ...
        if d in sig_by_date:
            ...
            if not paused:
                for code in sig_by_date[d][:50]:  # 每日最�?0个新买入
        ...
        if (day_idx + 1) % 100 == 0:
            ...
            print(f"  {d} | {day_idx+1}/{len(trading_dates)} | "
                  ...
                  f"日内退�?{intraday_exits} | EOD退�?{eod_exits} | 缺数�?{missing_data_count}")
    ...
    print(f"  �?最大回�? {md:>12.2f}%                          �?)
    print(f"  �?PF: {pf:>10.2f}  胜率: {wr:>7.1f}%                    �?)
    print(f"  �?总盈�? {tp:>+13,.0f}                           �?)
    print(f"  �?耗时: {elapsed:>10.0f}s  缺数据天�? {missing_data_count}              �?)
    print(f"  └─────────────────────────────────────────────────�?)
    ...
    print(f"  �?均持: {ah:>9.1f}�? 日内退�? {intra_count:>6}  EOD退�? {eod_count:>6}       �?)
    print(f"  └─────────────────────────────────────────────────�?)
