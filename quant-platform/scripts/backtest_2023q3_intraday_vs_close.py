#!/usr/bin/env python
"""MA5 策略 �?2023Q3~2026 回测：盘中监�?vs 尾盘�?""
...
def load_daily():
    ...
    print(f"{len(bars):,}�?{bars['code'].nunique()}�?{time.time()-t0:.0f}s"); return bars
...
def check_bar(pos, bar):
    ...
    if l<=pos.eprice*(1+HARD_STOP): return(pos.eprice*(1+HARD_STOP),"硬止�?,None)
...
def check_eod(pos,c,d):
    ...
    if hd>TIME_FORCE: return(c,f"时间强制({hd}�?")
    if hd>TIME_EXIT and cp>0.01: return(c,f"时间条件({hd}�?")
    return None
...
def run_mode(mode, sig, dd, tdates, label):
    """mode: 'intraday' (盘中监控) or 'close' (尾盘�?"""
    ...
    for di,d in enumerate(tdates):
        ...
        for code,pos in list(eng.pos.items()):
            ...
            if mode == 'intraday':
                # 盘中监控：用5分钟线逐根检�?                intra = load_min5_day(code, d)
                if not intra.empty:
                ...
                # 尾盘价：只用收盘价判断是否触发止�?止盈，按收盘价卖�?                sd=dd.get(d,{}).get(code)
                t = None
                if sd:
                    cp=sd['close']; cpp=cp/pos.eprice-1
                    # 更新峰值（用收盘价�?                    if cp>pos.peak: pos.peak=cp
                    pp=pos.peak/pos.eprice-1
                    # 1. 收盘价触发硬止损
                    if cp <= pos.eprice*(1+HARD_STOP):
                        t=eng.sell(pos,cp,"硬止�?,ed=d)
                        if t: eng.tr.append(t); eng.ie+=1; exited=True
                    # 2. 收盘价触发TP2
                    ...
                    if not exited and pp>=TRAIL_ACT and cp<=pos.peak*(1-TRAIL_DD):
                        ...
                        if t: eng.tr.append(t); eng.ie+=1; exited=True
                    # 5. 保本：曾盈利3%，收盘跌破成�?                    if not exited and pp>=0.03 and cp<=pos.eprice:
        ...
        # 买入（两种模式相同：次日开盘价买入�?        if d in sbd and (eng.pu is None or d>eng.pu):
        ...
        if (di+1)%150==0:
            eq=eng.eqv(epx)
            print(f"  {d} | {di+1}/{len(tdates)} | 净�?{eq:,.0f} | 持仓 {eng.np()} | 退�?{eng.ie+eng.ee} | �?min:{missing_5min} �?min:{used_5min}")

    elapsed=time.time()-t0
    print(f"  完成({elapsed:.0f}s) | 盘中退出{eng.ie} 尾盘退出{eng.ee} | �?min:{missing_5min} �?min:{used_5min}")
...
def report(ss):
    print("\n"+"�?*72)
    print("�? MA5 策略 �?盘中监控 vs 尾盘�?(2023Q3~2026)")
    print("�?*72)
    print(f"\n  {'指标':<20}",end="")
    for s in ss: print(f" {s['label']:>16}",end="")
    print("\n  "+"-"*56)
    for lbl,k,fmt in[("总成�?,"trades","d"),("胜率%","win_rate",".1f"),("总收�?","total_return",".2f"),("年化%","annual_return",".2f"),("最大回�?","max_dd",".2f"),("盈亏�?,"profit_factor",".2f"),("总盈�?,"total_profit",",.0f"),("均盈%","avg_win",".2f"),("均亏%","avg_loss",".2f"),("均持�?,"avg_hold",".1f"),("盘中退�?,"ie","d"),("尾盘退�?,"ee","d"),("最终净�?,"final_equity",",.0f"),("耗时s","elapsed",".0f")]:
        print(f"  {lbl:<20}",end="")
        for s in ss:
            v=s.get(k,0)
...
def main():
    t0=time.time()
    print("="*72); print("  MA5 策略 �?盘中监控 vs 尾盘�?(2023Q3~2026)"); print("="*72)
    ...
    print(f"{len(sig):,}信号 {sig['code'].nunique()}�?{time.time()-t1:.0f}s")
    ...
    print(f"{len(tdates)}�?{time.time()-t1:.0f}s")
    del bars,bt,sb; gc.collect()

    print("[4/5] 盘中监控...")
    eng_i,el_i,miss_i,use_i=run_mode('intraday',sig,dd,tdates,"盘中监控(5分钟�?")
    gc.collect()
    print("\n[5/5] 尾盘�?..")
    eng_c,el_c,_,_=run_mode('close',sig,dd,tdates,"尾盘价判�?)

    s_i=stats(eng_i,"盘中监控(5min)",el_i)
    s_c=stats(eng_c,"尾盘价判�?,el_c)
    s_i['missing_5min']=miss_i; s_i['used_5min']=use_i
    report([s_i,s_c])

    print(f"\n  盘中监控: �?分钟数据 {miss_i} �? 使用5分钟数据 {use_i} �?)
